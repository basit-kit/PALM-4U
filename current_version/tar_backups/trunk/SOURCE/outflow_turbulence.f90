...
! Former revisions:
! -----------------
! $Id: outflow_turbulence.f90 2051 2016-11-08 15:08:37Z gronemeier $
!
...
!------------------------------------------------------------------------------!
 SUBROUTINE outflow_turbulence
    ...
    USE arrays_3d,                                                             &
        ONLY:  e, pt, q, s, u, v, w, rs                                             !bK added rs

    USE control_parameters,                                                    &
        ONLY:  humidity, passive_scalar, outflow_source_plane, chemistry            !bK added chemistry
    ...
    USE indices,                                                               &
        ONLY:  nbgp, nx, nxr, ny, nyn, nys, nyng, nysg, nzb, nzt
    ...
    REAL(wp), DIMENSION(nzb:nzt+1,nysg:nyng,8,nbgp) ::                         &        !bK dim size inc fm 7 to 8
       ...
       DO  l = 1, nbgp
          DO  j = nysg, nyng
             DO  k = nzb, nzt + 1
                ...
                IF ( chemistry )                                                &   !bK added this line
                   outflow_val(k,j,8,l) = rs(k,j,i)                                !bK added this line
    ...
    IF ( nxr == nx )  THEN
       ...
       DO  j = nysg, nyng
          DO  k = nzb, nzt + 1
             ...
             IF ( chemistry )                                                  &    !bk added this line
                rs(k,j,nx+1:nx+nbgp)  = outflow_val(k,j,8,1:nbgp)                  !bK added this line
