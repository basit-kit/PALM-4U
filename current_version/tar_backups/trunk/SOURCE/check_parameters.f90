...
!       restart_time = remote
!       dt_restart= remote
!       simulation_time_since_reference = remote
!       dx = remote
...
!-- Check topography setting (check for illegal parameter combinations)
    IF ( topography /= 'flat' )  THEN
      ...
      .AND. scalar_advec /= 'ws-scheme-mono' )  THEN
...
!--          defined in init_grid.
             ...
             CALL message( 'check_parameters', 'PA0239', 1, 2, 0, 6, 0 )
          ELSE
...
!--          Set convention according to topography.
       ...
       ELSEIF ( TRIM( topography_grid_convention ) /= 'cell_edge' .AND.        &
          ...
          CALL message( 'check_parameters', 'PA0240', 1, 2, 0, 6, 0 )
       ENDIF
...
!-- Advection schemes:
    ...
    IF ( ( momentum_advec == 'ws-scheme' .OR.  scalar_advec == 'ws-scheme'     &
           .OR. scalar_advec == 'ws-scheme-mono' )                             &
    ...
    IF ( scalar_advec /= 'pw-scheme'  .AND.  scalar_advec /= 'ws-scheme' .AND. &
         scalar_advec /= 'ws-scheme-mono' .AND. scalar_advec /= 'bc-scheme' )  &
    THEN
    ...
    IF ( use_sgs_for_particles  .AND.  .NOT. cloud_droplets  .AND.               &
         ...
         ( scalar_advec /= 'ws-scheme'  .AND.  scalar_advec /= 'ws-scheme-mono' )&
       )  THEN
...
!-- Set LOGICAL switches to enhance performance
    ...
    IF ( scalar_advec   == 'ws-scheme' .OR.                                    &
         scalar_advec   == 'ws-scheme-mono' )  ws_scheme_sca = .TRUE.
    IF ( scalar_advec   == 'ws-scheme-mono' )  monotonic_adjustment = .TRUE.
...
!--    No restart run: several initialising actions are possible
    ...
    IF ( TRIM( initializing_actions ) == 'initialize_vortex'  .AND.            &
         ...
         message_string = 'initializing_actions = "initialize_vortex"' //      &
                        ' ist not allowed with conserve_volume_flow = .T.'
...
!   This IF clause needs revision, got too complex!!
    ...
    ELSE
        constant_heatflux = .TRUE.
        IF ( TRIM( initializing_actions ) /= 'read_restart_data'  .AND.        &
             large_scale_forcing  .AND.  .NOT.  land_surface )  THEN
           surface_heatflux = shf_surf(1)
        ENDIF
    ENDIF
       ...
       SELECT CASE ( TRIM( data_output_pr(i) ) )
          ...
          CASE ( 's', '#s' )
             ...
             ELSE
                dopr_index(i) = 117
                dopr_unit(i)  = 'kg/m3'
                hom(:,2,117,:) = SPREAD( zu, 2, statistic_regions+1 )
                IF ( data_output_pr(i)(1:1) == '#' )  THEN
                   dopr_initial_index(i) = 117
                   hom(:,2,117,:)        = SPREAD( zu, 2, statistic_regions+1 )
                   hom(nzb,2,117,:)      = 0.0_wp    ! because zu(nzb) is negative
                   data_output_pr(i)     = data_output_pr(i)(2:)
                ENDIF
             ENDIF
          ...
          CASE ( 'w"s"' )
             ...
             ELSE
                ...
                hom(:,2,119,:) = SPREAD( zw, 2, statistic_regions+1 )
             ENDIF
          ...
          CASE ( 'w*s*' )
             ...
             ELSE
                dopr_index(i) = 116
                dopr_unit(i)  = 'kg/m3 m/s'
                hom(:,2,116,:) = SPREAD( zw, 2, statistic_regions+1 )
             ENDIF
          ...
          CASE ( 's*2' )
             ...
             ELSE
                dopr_index(i) = 118
                dopr_unit(i)  = 'kg2/kg2'
                hom(:,2,118,:) = SPREAD( zu, 2, statistic_regions+1 )
             ENDIF
...
!    if (myid==0) print*,'fm #2_0st instance print var ... ', data_output
    DO  WHILE ( data_output(i) /= ' '  .AND.  i <= 500 )
!        if (myid==0) print*,'fm #2_1st, dataout(i) ... ', i, data_output(i)
!
!--    Check for data averaging
       ...
       IF ( ilen > 3 )  THEN
          ...
          ENDIF
       ENDIF
!       if (myid==0) print*,'fm #2_2nd, chem spcs included in data_output here ...', data_output(i)            !bK debug
!
!--    Check for cross section or volume data
       ...
       IF ( ilen > 3 )  THEN
          IF ( data_output(i)(ilen-2:ilen) == '_xy'  .OR.                      &
             ...
             k = 1                                             ! 2d data
!         print*,'fm #2_3rd, ilen, data_output(i), var...',ilen, data_output(i),var
...
!    print*,'fm #2_4th instance print var .... ', var
!
...
!--       Check, if variable is already subject to averaging
          ...
          DO  k = 1, doav_n
             ...
             if(myid==0) print*,'print doav_n : ', doav_n
             if(myid==0) print*,'print doav(k) : ',doav(k)
          ENDDO
...
!--    not allowed for parallel output.
       ...
       ntdim_3d(0)    = INT( ( end_time - skip_time_do3d ) / dt_do3d )
       IF ( do3d_at_begin ) ntdim_3d(0) = ntdim_3d(0) + 1
       ntdim_3d(1)    = INT( ( end_time - skip_time_data_output_av )           &
                             / dt_data_output_av )
       ntdim_2d_xy(0) = INT( ( end_time - skip_time_do2d_xy ) / dt_do2d_xy )
       ntdim_2d_xz(0) = INT( ( end_time - skip_time_do2d_xz ) / dt_do2d_xz )
       ntdim_2d_yz(0) = INT( ( end_time - skip_time_do2d_yz ) / dt_do2d_yz )
       IF ( do2d_at_begin )  THEN
          ntdim_2d_xy(0) = ntdim_2d_xy(0) + 1
          ntdim_2d_xz(0) = ntdim_2d_xz(0) + 1
          ntdim_2d_yz(0) = ntdim_2d_yz(0) + 1
       ENDIF
       ntdim_2d_xy(1) = ntdim_3d(1)
...
!-- potential temperature, check the width of the damping layer
    IF ( bc_lr /= 'cyclic' ) THEN
       IF ( pt_damping_width < 0.0_wp  .OR.                                    &
            pt_damping_width > REAL( nx * dx ) )  THEN
    ...
    IF ( bc_ns /= 'cyclic' )  THEN
       IF ( pt_damping_width < 0.0_wp  .OR.                                    &
            pt_damping_width > REAL( ny * dy ) )  THEN
          message_string = 'pt_damping_width out of range'
          CALL message( 'check_parameters', 'PA0124', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF
       ...
       IF ( dt_do == 0.0_wp )  THEN
          IF ( dt_fixed )  THEN
             WRITE( message_string, '(A,F9.4,A)' )  'Output at every '  //     &
                    'timestep is desired (' // dt_do_name // ' = 0.0).&'//     &
                    'Setting the output interval to the fixed timestep '//     &
                    'dt = ', dt, 's.'
       ...
       IF ( .NOT. land_surface  .AND.  .NOT. large_scale_forcing )  THEN
          IF ( ibc_b == 0  .AND.  constant_flux )  THEN
             message_string = 'boundary condition: bc_' // TRIM( sq ) // '_b ' //  &
                              '= "' // TRIM( bc_b ) // '" is not allowed with ' // &
                              'prescribed surface flux'
