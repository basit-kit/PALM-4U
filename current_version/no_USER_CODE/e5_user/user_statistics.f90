...
!--    velocities u and v.
       ...
       DO  i = 1, size_of_pos_array
          ...
          IF ( nxl <= s_ts_pos_x_ind(i)  .AND.  nxr >= s_ts_pos_x_ind(i) )  THEN
             IF ( nys <= s_ts_pos_y_ind(i)  .AND.  nyn >= s_ts_pos_y_ind(i) )  THEN

                ts_value_l(dots_num_palm+i) = s(2,s_ts_pos_y_ind(i),s_ts_pos_x_ind(i))
