...
! Former revisions:
! -----------------
! $Id: read_var_list.f90 2425 2017-09-11 14:21:39Z basit $
!
 ...
 SUBROUTINE read_var_list
    ...
    USE arrays_3d,                                                             &
        ONLY:  inflow_damping_factor, mean_inflow_profiles, pt_init,           &
               q_init, ref_state, rs_init, s_init, sa_init, u_init, ug, v_init, vg    !bK added rs_init
...
!-- which are needed for the following read instructions.
    ...
    IF ( .NOT. ALLOCATED( ug ) )  THEN
       ALLOCATE( ug(0:nz+1), u_init(0:nz+1), vg(0:nz+1),                       &
                 ...
                 ref_state(0:nz+1), rs_init(0:nz+1),  s_init(0:nz+1), sa_init(0:nz+1),           &    !bK added rs_init
...
!--          increased. The same changes must also be done in write_var_list.
    ...
    DO  WHILE ( TRIM( variable_chr ) /= '*** end ***' )
       ...
       SELECT CASE ( TRIM( variable_chr ) )
          ...
          CASE ( 'run_coupled' )
             READ ( 13 )  run_coupled
          CASE ( 'rs_init' )                                !bK added case nd read for rs_*
             READ ( 13 )  rs_init
          CASE ( 'rs_surface' )
             READ ( 13 )  rs_surface
          CASE ( 'rs_surface_initial_change' )
             READ ( 13 )  rs_surface_initial_change
          CASE ( 'rs_vertical_gradient' )
             READ ( 13 )  rs_vertical_gradient
          CASE ( 'rs_vertical_gradient_level' )
             READ ( 13 )  rs_vertical_gradient_level
          CASE ( 'rs_vertical_gradient_level_ind' )
             READ ( 13 )  rs_vertical_gradient_level_ind 
          CASE ( 's_init' )
             READ ( 13 )  s_init
          CASE ( 's_surface' )
             READ ( 13 )  s_surface
