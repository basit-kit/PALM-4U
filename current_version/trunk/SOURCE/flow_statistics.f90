...
! Former revisions:
! -----------------
! $Id: flow_statistics.f90 2382 2017-09-01 12:20:53Z basit $
!
 ...
 SUBROUTINE flow_statistics
    ...
    USE arrays_3d,                                                             &
        ONLY:  ddzu, ddzw, e, heatflux_output_conversion, hyp, km, kh,         &
               ...
               qc, ql, qr, qs, qsws, qswst, rho_air, rho_air_zw, rho_ocean,    &
               rs, rss, rssws, rsswst,                                         &           !bK added rs, rss, rssws, rsswst 
               s, sa, ss, ssws, sswst, saswsb, saswst, shf, td_lsa_lpt, td_lsa_q, &
...
!--                   Formula does not work if ql(nzb) /= 0.0
          ...
          IF ( chemistry )  THEN                                    !bK added this IF block
             ...
             DO  i = nxl, nxr
                DO  j =  nys, nyn
                   s1 = s1 + rssws(j,i) * rmask(j,i,sr)  ! w"s"
                ENDDO
             ENDDO
             sums_l(nzb,119,tn) = s1
             !$acc end parallel
...
!--                   Formula does not work if ql(nzb) /= 0.0
          ...
          IF ( chemistry )  THEN                        !bK added this IF block
             ...
             DO  i = nxl, nxr
                DO  j =  nys, nyn
                   s1 = s1 + rsswst(j,i) * rmask(j,i,sr) ! w"s"
                ENDDO
             ENDDO
             sums_l(nzt,119,tn) = s1
             !$acc end parallel
