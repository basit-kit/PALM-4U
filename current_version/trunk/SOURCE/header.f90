...
!-----------------------------------------------------------------------------!
 SUBROUTINE header
    ...
    USE arrays_3d,                                                             &
        ONLY:  pt_init, qsws, q_init, rs_init,  s_init, sa_init, shf,          &   !bK added rs_init
               ug, vg, w_subs, zu, zw
...
!-- Numerical schemes
    ...
    ELSEIF ( scalar_advec == 'ws-scheme' )  THEN
       WRITE ( io, 504 )
    ELSEIF ( scalar_advec == 'ws-scheme-mono' )  THEN
       WRITE ( io, 513 )
    ELSE
       WRITE ( io, 118 )
    ENDIF
...
!-- Boundary conditions
    ...
    IF ( air_chemistry )  THEN                          !bK added if block
       ...
       ENDIF
       IF ( ibc_rs_t == 0 )  THEN
          r_upper =  'rs(nzt+1) = s_top'
       ELSEIF ( ibc_rs_t == 1 )  THEN
          r_upper =  'rs(nzt+1) = rs(nzt)'
       ELSEIF ( ibc_rs_t == 2 )  THEN
          r_upper =  'rs(nzt+1) = rs(nzt) + ds/dz'
       ENDIF
       WRITE ( io, 301 ) 'rs', r_lower, r_upper
    ENDIF
...
!-- Building output strings, starting with surface humidity
    IF ( passive_scalar )  THEN
       ...
       DO  WHILE ( s_vertical_gradient_level_ind(i) /= -9999 )
          ...
          WRITE (coor_chr,'(E8.1,4X)')  s_init(q_vertical_gradient_level_ind(i))
          temperatures = TRIM( temperatures ) // '  ' // TRIM( coor_chr )
...
!--                                                                                  !bK added this if block
    IF ( air_chemistry )  THEN
       ...
       coordinates = '     0.0'
       i = 1
       DO  WHILE ( rs_vertical_gradient_level_ind(i) /= -9999 )
          
          WRITE (coor_chr,'(E8.1,4X)')  rs_init(q_vertical_gradient_level_ind(i))
          temperatures = TRIM( temperatures ) // '  ' // TRIM( coor_chr )

          WRITE (coor_chr,'(E8.1,4X)')  rs_vertical_gradient(i)
          gradients = TRIM( gradients ) // '  ' // TRIM( coor_chr )
          
          WRITE (coor_chr,'(I8,4X)')  rs_vertical_gradient_level_ind(i)
          slices = TRIM( slices ) // '  ' // TRIM( coor_chr )
          
          WRITE (coor_chr,'(F8.1,4X)')  rs_vertical_gradient_level(i)
          coordinates = TRIM( coordinates ) // '  '  // TRIM( coor_chr )
       ...
       WRITE ( io, 422 )  TRIM( coordinates ), TRIM( temperatures ),           &
                          TRIM( gradients ), TRIM( slices )                         !bk check if this is ok
    ENDIF    
