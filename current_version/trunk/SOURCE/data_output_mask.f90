...
! Former revisions:
! -----------------
! $Id: data_output_mask.f90 2032 2016-10-21 15:13:51Z knoop $
!
...
#if defined( __netcdf )
    USE arrays_3d,                                                             &
        ONLY:  e, nr, p, pt, q, qc, ql, ql_c, ql_v, qr, rho_ocean, rs, s, sa, tend, u,   &      !bK added rs
               v, vpt, w
    
    USE averaging,                                                             &
        ONLY:  e_av, lpt_av, nr_av, p_av, pc_av, pr_av, pt_av, q_av, qc_av,    &
               ql_av, ql_c_av, ql_v_av, ql_vp_av, qv_av, qr_av, rho_ocean_av, rs_av,  s_av,  &   !bK added rs_av
...
!--    Reallocate local_pf on PE 0 since its shape changes during MPI exchange
       IF ( netcdf_data_format < 5   .AND.  myid == 0  .AND.  if > 1 )  THEN
          DEALLOCATE( local_pf )
          ALLOCATE( local_pf(mask_size_l(mid,1),mask_size_l(mid,2), &
                             mask_size_l(mid,3)) )
       ENDIF
...
!--          Radiation quantity
             IF ( radiation )  THEN
                CALL radiation_data_output_mask(av, domask(mid,av,if), found,  &
                                                local_pf )
             ENDIF
...
!--          User defined quantity
             IF ( .NOT. found )  THEN
                CALL user_data_output_mask(av, domask(mid,av,if), found,       &
                                           local_pf )
             ENDIF
             ...
             IF ( .NOT. found )  THEN
                WRITE ( message_string, * ) 'no output available for: ', &
                                            TRIM( domask(mid,av,if) )
...
!--             Not all PEs have data for the mask
             ...
             nc_stat = NF90_PUT_VAR( id_set_mask(mid,av),  &
                  id_var_domask(mid,av,if), total_pf, &
