...
! Former revisions:
! ------------------
! $Id: init_ocean.f90 2425 2017-09-11 14:21:39Z basit $
!
...
!------------------------------------------------------------------------------!
 SUBROUTINE init_ocean
    ...
    USE control_parameters,                                                    &
        ONLY:  g, prho_reference, rho_surface, rho_reference,                  &
...
!-- and pressure (based on in situ density)
    DO  n = 1, 5
       ...
       DO  k = nzt-1, 0, -1
          hyp(k) = hyp(k+1) + g * 0.5_wp * ( rho_ocean_init(k) + rho_ocean_init(k+1 ) ) * &
                              dzu(k+1)
       ENDDO
