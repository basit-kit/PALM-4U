...
! Former revisions:
! -----------------
! $Id: parin.f90 2055 2016-11-09 09:30:24Z ketelsen $
! 
...
!------------------------------------------------------------------------------!
 SUBROUTINE parin
    ...
    USE arrays_3d,                                                             &
        ONLY:  pt_init, q_init, ref_state, rs_init, s_init, sa_init,           &  !bK added rs_init   
               ug, u_init, v_init, vg
    ...
    NAMELIST /inipar/  alpha_surface, approximation,                           &
             ...
             roughness_length, rs_surface, sa_surface,                         &    !bK added rs_surface
...
!--                  also check the allocate statement in routine read_var_list
             ALLOCATE( pt_init(0:nz+1), q_init(0:nz+1), rs_init(0:nz+1),       &        !bK added rs_init
                       s_init(0:nz+1), ref_state(0:nz+1), sa_init(0:nz+1),     &
