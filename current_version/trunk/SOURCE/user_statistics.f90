...
!------------------------------------------------------------------------------!
 SUBROUTINE user_statistics( mode, sr, tn )
    ...
    INTEGER(iwp) ::  sr   !< 
    INTEGER(iwp) ::  tn   !< 
    INTEGER(iwp) :: n,m   !< bK added counters for chem_species loop  
    REAL(wp),                                                                  &
...
!!--             horizontal momentum flux u*v*
                DO n = 1, NVAR                                                                   !bK added
                 sums_l(k,pr_palm+1,tn) = sums_l(k,pr_palm+1,tn) +                         &
                        chem_species(5)%conc_p(k,j,i) * 0.5_wp * ( w(k,j,i) + w(k,j+1,i) ) &
                                                 * rmask(j,i,sr)
                sums_l(k,pr_palm+2,tn) = sums_l(k,pr_palm+2,tn) +                          &
                        chem_species(5)%conc_p(k,j,i)  * rmask(j,i,sr)
!!         
