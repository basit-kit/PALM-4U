 ...
 SUBROUTINE sum_up_3d_data
    ...
    USE arrays_3d,                                                             &
        ONLY:  dzw, e, nr, ol, p, pt, precipitation_rate, q, qc, ql, ql_c,     &
               ql_v, qr, qsws, rho_ocean, rs, s, sa, shf, ssws, ts, u, us, v, vpt, w, z0,&  !bK added rs
               z0h, z0q

    USE averaging,                                                             &
        ONLY:  e_av, lpt_av, lwp_av, nr_av, ol_av, p_av, pc_av, pr_av, prr_av, &
               precipitation_rate_av, pt_av, q_av, qc_av, ql_av, ql_c_av,      &
               ql_v_av, ql_vp_av, qr_av, qsws_av, qv_av, rho_ocean_av, rs_av, s_av, sa_av,  &   !bK added rs_av
    ...
    INTEGER(iwp) ::  ii, nn  !< running index
    INTEGER(iwp) ::  j   !< running ind
    ...
    CHARACTER (LEN=varnamelength) ::  trimvar,spec_name  !< TRIM of output-variable string
...
!if(myid==0) print*,'print chem_spcs%name',chem_species(:)%name
          ...
          SELECT CASE ( trimvar )
             ...
             CASE ( 's' )
                if(myid==0) write(9,*) 'fm 75, case s-2', ii
...
!              CASE (spec_name)
!                if(myid==0) write(9,*) 'fm 75.1 sum_up.., chem_species(n)%name),ii is .. ',ii,' ..and trimvar is.. ',trimvar             
!                flush(9)
!                IF ( .NOT. ALLOCATED( rs_av ) )  THEN
!                    print*,'fm #22  allocate rs_av now!'
!                    ALLOCATE( rs_av(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )        !bK rs_n:total number of reactive scalars
!                ENDIF
!                rs_av = 0.0_wp
             ...
             CASE ( 'rssws*' )                                      !bK added this case block
...
!         CASE ('rs')
!         print*,' fm rs loop 2 , val of ii is  : ',ii    !bK debug
!         flush(9)
!
!             DO  i = nxlg, nxrg
!                DO  j = nysg, nyng
!                   DO  k = nzb, nzt+1
!                      rs_av(k,j,i) = rs_av(k,j,i) + rs(k,j,i)
!                   ENDDO
!                ENDDO
!             ENDDO
          ...
          CASE ( 'rssws*' )                                     !bK added this CASE block
             DO  i = nxlg, nxrg
                DO  j = nysg, nyng
                   rssws_av(j,i) = rssws_av(j,i) + rssws(j,i)
                ENDDO
             ENDDO
