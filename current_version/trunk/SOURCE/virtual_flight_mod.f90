...
! Former revisions:
! -----------------
! $Id: virtual_flight_mod.f90 2425 2017-09-11 14:21:39Z basit $
!
...
! Bugfix concerning labeling of timeseries.
! 
...
!--------------------------------------------------------------------------------!
 MODULE flight_mod
    ...
    USE control_parameters,                                                    &
        ONLY:  fl_max, num_leg, num_var_fl, num_var_fl_user, virtual_flight
    ...
    INTERFACE flight_skip_var_list 
       MODULE PROCEDURE flight_skip_var_list
    END INTERFACE flight_skip_var_list
    
    INTERFACE flight_read_restart_data 
       MODULE PROCEDURE flight_read_restart_data
    END INTERFACE flight_read_restart_data
    
    INTERFACE flight_write_restart_data 
       MODULE PROCEDURE flight_write_restart_data 
    END INTERFACE flight_write_restart_data 
...
!-- Public interfaces
    PUBLIC flight_init, flight_header, flight_parin, flight_measurement,       &
           flight_skip_var_list, flight_read_restart_data,                     &
           flight_write_restart_data
!
!-- Public variables
       ...
       DO l=1, num_leg
          ...
          DO i=1, num_var_fl
             ...
             ELSEIF ( i == id_w  )  THEN          
                ...
                k             = k + 1
             ELSEIF ( i == id_pt )  THEN       
                dofl_label(k) = TRIM( label_leg ) // '_pt'
                dofl_unit(k)  = 'K'
                k             = k + 1
             ELSEIF ( i == id_q  )  THEN       
...
!------------------------------------------------------------------------------!
    SUBROUTINE flight_measurement
       ...
       USE control_parameters,                                                 &
           ONLY:  cloud_droplets, cloud_physics, dz, dz_stretch_level, dt_3d,  &
       ...
       USE indices,                                                            &
           ONLY:  nx, nxl, nxlg, nxr, nxrg, ny, nys, nysg, nyn, nyng, nzb, nzt
...
!--    fac=1 if variable is on scalar grid level, fac=0 for w-component.
       IF ( z_pos(l) < dz_stretch_level )  THEN
          k = ( z_pos(l) + fac * 0.5_wp * dz ) / dz
       ELSE
!
!--       Search for k-index
          DO k1=nzb, nzt
...
!------------------------------------------------------------------------------!
    SUBROUTINE flight_check_parameters
       ...
       USE control_parameters,                                                 &
           ONLY:  bc_lr_cyc, bc_ns_cyc, dz, message_string
...
!..       return-legs.  
       ...
       ENDDO          
!
!--    Check if start and end positions are properly set in case of return legs.
       DO l=1, num_leg

          IF ( x_start(l) > x_end(l) .AND. leg_mode(l) == 'return' )  THEN
             message_string = 'x_start position must be <= x_end ' //          &
                              'position for return legs'
             CALL message( 'flight_check_parameters', 'PA0436', 1, 2, 0, 6, 0 )
          ENDIF 
          IF ( y_start(l) > y_end(l) .AND. leg_mode(l) == 'return' )  THEN
             message_string = 'y_start position must be <= y_end ' //          &
                              'position for return legs'
             CALL message( 'flight_check_parameters', 'PA0437', 1, 2, 0, 6, 0 )
          ENDIF 
       ENDDO
!
...
! Description:
! ------------
!> Skipping the flight-module variables from restart-file (binary format).
!------------------------------------------------------------------------------!
    SUBROUTINE flight_skip_var_list  
       ...
       DO  WHILE ( TRIM( variable_chr ) /= '*** end flight ***' )
          READ ( 13 )  cdum
          READ ( 13 )  variable_chr
       ENDDO    
       
    END SUBROUTINE flight_skip_var_list  
...
!------------------------------------------------------------------------------!
    SUBROUTINE flight_read_restart_data  
    ...
    END SUBROUTINE flight_read_restart_data  
...
!------------------------------------------------------------------------------!
    SUBROUTINE flight_write_restart_data  
    ...
    END SUBROUTINE flight_write_restart_data    
