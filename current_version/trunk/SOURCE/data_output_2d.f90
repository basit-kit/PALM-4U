...
! Former revisions:
! -----------------
! $Id: data_output_2d.f90 2425 2017-09-11 14:21:39Z basit $
!
 ...
 SUBROUTINE data_output_2d( mode, av )
    ...
    USE arrays_3d,                                                             &
        ONLY:  dzw, e, nr, ol, p, pt, precipitation_amount, precipitation_rate,&
               prr,q, qc, ql, ql_c, ql_v, ql_vp, qr, qsws, rho_ocean, rs, rssws, s, sa, shf,    &  !bK added rs, rssws
    ...
    USE control_parameters,                                                    &
        ONLY:  cloud_physics, data_output_2d_on_each_pe, data_output_xy,       &
               ...
               do2d_xy_last_time, do2d_xy_n, do2d_xy_time_count,               &
    ...
    USE netcdf_interface,                                                      &
        ONLY:  id_set_xy, id_set_xz, id_set_yz, id_var_do2d, id_var_time_xy,   &
               id_var_time_xz, id_var_time_yz, nc_stat, netcdf_data_format,    &
               netcdf_handle_error
...
!--       Store the array chosen on the temporary array.
          ...
          SELECT CASE ( TRIM( do2d(av,if) ) )
             ...
             CASE ( 'rs_xy', 'rs_xz', 'rs_yz' )         !bK added CASE block
                IF ( av == 0 )  THEN
                   to_be_resorted => rs
                ELSE
             ...
             CASE ( 'rssws*_xy' )        ! 2d-array                 !bK added this CASE block
                IF ( av == 0 ) THEN
                   DO  i = nxlg, nxrg
                      DO  j = nysg, nyng
                         local_pf(i,j,nzb+1) =  ssws(j,i)
                ...
                ELSE
                   DO  i = nxlg, nxrg
                      DO  j = nysg, nyng 
                         local_pf(i,j,nzb+1) =  rssws_av(j,i)
                      ENDDO
                   ENDDO
                ENDIF
                resorted = .TRUE.
