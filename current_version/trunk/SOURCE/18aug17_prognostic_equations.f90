...
!>
!> @todo tendency_caller needs to be a subroutine in schem_driver
 ...
 MODULE prognostic_equations_mod
    ...
    USE arrays_3d,                                                             &
        ONLY:  diss_l_e, diss_l_nr, diss_l_pt, diss_l_q, diss_l_qr,            &
               ...
               rdf_sc, ref_state, rho_ocean, rs, rs_init, rssws, rsswst,       &    !bK added rs, rs_init, rssws, rsswst
               s, s_init, s_p, sa, sa_init, sa_p,                              &
       ...
       DO  i = nxl, nxr
          DO  j = nys, nyn
            if(myid == 0) print*,'fm prong_eqn AFTER calling kchem_integrate    #9.1 '       !bK debug
             ...
             CALL kchem_integrate (i,j)      ! TEST RFo                                          
          ENDDO
       ENDDO
...
!                IF (myid == 2 .and. k==2 .and. j == 10 .and. i == 30) print*, ' *** val of s_p array at codeflow point-1 is :', s_p(k,j,i) 
!                flush(9)

!                 IF (myid == 2 ) print*, ' vals of i,j index are : ',i,j
!               IF (myid == 2 ) print*, ' *** val of nzb_s_inner, nzt :', nzb_s_inner(j,i), nzt 
!               flush(9)
...
!                if(myid == 0  ) print*, 'fm prog_eqn, BEFORE calling tendency_caller #14.1 '  !bK debug
                ...
                CALL tendency_caller ( chem_species(n)%conc_p, chem_species(n)%conc,   &
                                       ...
                                       i, j, i_omp_start, tn,                          &
                                       chem_species(n)%rssws, chem_species(n)%rsswst,  &            !bK replaced ssws and sswst  with rssws abd rsswst respectively.
                                       chem_species(n)%flux_s, chem_species(n)%diss_s, &
                                       chem_species(n)%flux_l, chem_species(n)%diss_l)       

                
!                                       rs_p(:,:,:,n)   => chem_species(n)%conc_p
!                                       rs  (:,:,:,n)   => chem_species(n)%conc
!                                       trs (:,:,:,n)   => chem_species(n)%tconc_m
...
!        IF(myid == 2) print*,'fm prog-eqn, AFTER Calling tendency_caller',myid                 !bK
#endif
...
!--                Tendencies for the next Runge-Kutta step
 ...
 SUBROUTINE tendency_caller ( rscalar_p, rscalar, trscalar_m, i, j, i_omp_start, tn, ssws_kc, sswst_kc, flux_s, diss_s, flux_l, diss_l )
    USE pegrid        !myid added by bK 
    USE indices,   ONLY: nysg, nyng, nxlg, nxrg

    IMPLICIT NONE

    REAL(wp), DIMENSION(:,:,:), POINTER                         :: rscalar_p, rscalar, trscalar_m
...
!-- local variables
    ...
    LOGICAL :: test
    rs_init = 0.001                                                  !bK added
    
!if(myid == 0) print*,'from prog-eqn, entering tendecy_caller'       !bK debug 
...
!-- Advection terms
    IF ( timestep_scheme(1:5) == 'runge' )  THEN
       IF ( ws_scheme_sca )  THEN
!        if(myid == 0) print*,'fm tend_caller, calling advec.. #15.1'       !bK debug 
...
!    if(myid == 0) print*,'fm tend_caller, calling diffusion  #16.1'
    CALL diffusion_s( i, j, rscalar, ssws_kc, sswst_kc, wall_sflux )   !kk use ssws and sswst instead of sswrc and sswrct   OK?

!if(myid == 0) print*,'from prog-eqn-tendency_caller, breakpoint 2, after diffusion'       !bK debug 

!    
!-- Prognostic equation for scalar
    DO  k = nzb_s_inner(j,i)+1, nzt
       rscalar_p(k,j,i) = rscalar(k,j,i) + dt_3d  * ( tsc(2) * tend(k,j,i) + tsc(3) * trscalar_m(k,j,i) )            &
                               - tsc(5) * rdf_sc(k) * ( rscalar(k,j,i) - rs_init(k) )  !kk rs_init wird s_init OK?      !bK replaced s_init with rs_init
       IF ( rscalar_p(k,j,i) < 0.0_wp )  rscalar_p(k,j,i) = 0.1_wp * rscalar(k,j,i)    !FKS6
    ENDDO

!if(myid == 0) print*,'from prog-eqn-tendency_caller, breakpoint 3, after prognostic_calc'       !bK debug 
...
!-- Calculate tendencies for the next Runge-Kutta step
    IF ( timestep_scheme(1:5) == 'runge' )  THEN
       IF ( intermediate_timestep_count == 1 )  THEN
          DO  k = nzb_s_inner(j,i)+1, nzt
             trscalar_m(k,j,i) = tend(k,j,i)
          ENDDO
       ELSEIF ( intermediate_timestep_count < &
          intermediate_timestep_count_max )  THEN
          DO  k = nzb_s_inner(j,i)+1, nzt
             trscalar_m(k,j,i) = -9.5625_wp * tend(k,j,i) + &
                5.3125_wp * trscalar_m(k,j,i)
          ENDDO
       ENDIF
    ENDIF


!        rs_p   => rscalar_p        ! bK added
!        rs     => rscalar          ! bK added
!        trs_m  => trscalar_m       ! bK added


 END SUBROUTINE tendency_caller
